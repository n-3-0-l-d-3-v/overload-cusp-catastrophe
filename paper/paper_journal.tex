%% =====================================================================
%% When Cusp Geometry Is an Artefact (long-form journal draft)
...
\title{When Cusp Geometry Is an Artefact: Derived Scaling Laws,
Non-Identifiability, and a Ground-Truth Failure of Rolling-Window
...
We present a cusp-catastrophe formulation of sensory--executive overload
together with an identifiability analysis that bounds what any such analysis
can recover from wearable data, and a ground-truth test of the early-warning
estimators the formulation would otherwise be checked with.
...
What the general stress corpora can settle is not whether autistic overload is
a cusp, but whether the machinery could establish that it is. They carry the
same sensor, the same sampling rate and the same recording lengths the intended
study would, so the identifiability bound in Sec.~\ref{sec:recovery} and the
estimator failure in Sec.~\ref{sec:results} transfer to that study directly.
No claim about the population transfers, and none is made.
...
\begin{center}
\footnotesize
\begin{tabular}{@{}l p{0.29\columnwidth} p{0.30\columnwidth}@{}}
\toprule
...
\begin{center}
\footnotesize
\begin{tabular}{@{}l p{0.22\columnwidth} p{0.27\columnwidth} l@{}}
\toprule
config & $x$ & drivers & role\\
\midrule
B & tonic EDA & HR, HR-uncertainty, ACC & primary\\
C & skin temperature & SCR rate, EDA-uncertainty, ACC & robust.\\
A & $z(\mathrm{HR})$ ${-}\,z(\mathrm{RMSSD})$ & as C & control\\
\bottomrule
\end{tabular}
\end{center}
...
that reported bistability there would report it for anything. Both comparisons
are internal to this paper: the cardiac index is a reference channel used to
justify a sensor assignment, not a competing method, and no published system is
claimed to be outperformed anywhere in what follows.
...
$n\in\{100,150,200,300,500,1000\}$ at 2000 replicates per cell for parameter
recovery ($1.2\times10^{4}$ series), 5000 for size ($30{,}000$ nominal and
$7{,}500$ calibrated) and 100 for power; the early-warning validation covers 90
configurations --- five window widths, two detrending choices, three lengths to
$n=20{,}000$ --- at 500 replicates each ($1.5\times10^{4}$ series). Replication
counts were fixed in advance to hold the Monte Carlo standard error on a
proportion below $0.01$ for size and power and below $0.02$ elsewhere. The
exception is the AR(1) persistence study at 40 replicates per point, which is
why it is quoted as a bound rather than an estimate. The suite runs on one
machine: ten worker processes on a six-core, twelve-thread CPU, roughly seven
hours of wall time for a full replication.

\subsection{Reproducibility}
All analyses are seeded from a single master seed and run from a single entry
point. The estimator is
...
\subsection{What the estimator recovers, and at what length}
\label{sec:recovery}
On long series the estimator behaves impeccably. Across 30 simulated units at
$n=4000$ windows it recovers eight of nine parameters with correlations above
$0.95$ ($\alpha_0$ at $r=0.98$, $\lambda$ at $r=0.997$, $\sigma$ at $r=0.999$),
absolute biases below $0.02$, and only the slow rate $\varepsilon$ lagging at
$r=0.51$.

That number is reported here because it is the one an earlier draft of this
work led with, and it is close to meaningless as a description of what these
corpora support. Two things make it optimistic. The series are $4000$ windows
long against a median of $194$ in the data, and the true $\lambda$ is drawn
from $[0.10,0.28]$ against a median of $0.0059$ in the real fits. The estimator
was being asked an easy question.

Asked the honest one --- randomised truth, 2000 replicates at each of six
lengths spanning the observed range, $1.2\times10^{4}$ series in all --- the
picture changes completely, and it is that study rather than this paragraph
that should be read as the result. At the median observed length only $\lambda$ ($\rho=0.94$) and $\sigma$
($\rho=0.99$) are recovered on both rank and value. The splitting factor sits
at $\rho=0.57$ and $r=0.15$.

Worse, the two measures never reconverge. By $n=1000$ the rank recovery of
$\alpha_0$ reaches $\rho=0.88$ while its value recovery is $r=0.10$, because
$a=\theta_2/\theta_1$ and nothing bounds $\theta_1$ away from zero: the
recovered $\hat\alpha_0$ spans $[-140,3.3]$ against a $99$th percentile of
$2.0$, so one replicate in two thousand destroys the linear correlation.
Dividing by a small number preserves order and destroys scale. Length therefore
buys a defensible ranking of participants by $\alpha_0$ and never buys a
reportable value.

Every conclusion depending on $\varepsilon$ or $\alpha_A$ --- principally P3 to
P5 --- is reported as exploratory, and on this evidence cannot be made
confirmatory by collecting longer recordings alone.
...
A & $z(\mathrm{HR}){-}z(\mathrm{RMSSD})$ & neg. control & $0.051$ \\
\bottomrule
\end{tabular}
\end{center}
...
generative AI, the authors disclose that an AI assistant was used for code
implementation, literature organisation and manuscript drafting; the authors
verified all analyses, results and claims and take full responsibility for the
content.
